...
class AccountAuthCreateService(
	...
	suspend fun reIssueTokenResponse(ip: String, refresh: String): TokenResponse {
		val userId = jwtProvider.getUserId(refresh, Token.REFRESH)
		val inRedisToken = redisProvider.getData(userId)
		
		//토큰이 옳바른지, redis에 존재하는지를 확인
		if (!jwtProvider.isValid(refresh, ip, Token.REFRESH) || inRedisToken != refresh) {
		...
		val authorities = jwtProvider.getSimpleGrantedAuthority(refresh, Token.REFRESH)
		
		return createTokenResponse(AccountLoginDTO(userId, authorities), ip)
	}
	
	
	//token response를 만들어 주는 함수
	suspend fun createTokenResponse(dto: AccountLoginDTO, ip: String): TokenResponse {
		//토큰 날짜를 통일 시키기 위해 현재 시간을 저장
		val issuedAt = Date(System.currentTimeMillis())
		val tokenResponse = jwtProvider.generate(
			dto.userId,
			dto.authorities, ip, issuedAt
		)
