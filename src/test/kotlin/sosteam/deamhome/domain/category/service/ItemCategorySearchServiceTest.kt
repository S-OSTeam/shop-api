...
class ItemCategorySearchServiceTest : BehaviorSpec({
	...
	Given("a title") {
		...
		When("finding item categories containing the title") {
			coEvery { itemCategoryRepository.findItemCategoriesContainTitle(title) } returns flowOf(
				itemCategory1,
				itemCategory2
			)
	...
	Given("all item categories") {
		...
		When("finding all item categories in tree format") {
			coEvery { categoryProvider.findAllCategoriesTree(any()) } returns listOf(
				categoryTreeResponse1,
				categoryTreeResponse2
			)
